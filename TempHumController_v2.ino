...
int RotaryValue = 26; // 로터리 엔코더 누적 값.
...
void loop() {
  ...
  //Serial.print("Temperature:"); Serial.print(t); Serial.print("C Target: "); Serial.print(dt);
...
//  Serial.print("% Adjust: "); Serial.println(h_const);
  ...
  if (isnan(Tempe) || isnan(Humdt)) {
    ...
    // 온도 표시 LCD 출력    
    lcd.setCursor (0,0); lcd.print("Temp: ");lcd.print(Tempe);lcd.print("/");
    lcd.print(TargetTempe);
    
    if (SettingMode == TEMPERATE_SETTING_MODE) lcd.print ("C_[");
    else lcd.print ("C [");
    ...
    // 습도 표시 LCD 출력
    lcd.setCursor (0,1); lcd.print("Humi: ");lcd.print(Humdt);lcd.print("/");lcd.print(TargetHumdt);
    if (SettingMode == HUMIDITY_SETTING_MODE) lcd.print ("%_[");
    else lcd.print ("% [");
    if (Humdt < TargetHumdt) {
      // Puzzy Theary
      Distance = (double) Humdt / (double) TargetHumdt;
      //Serial.println (Distance);
      if (Distance < HUMDT_PUZZY_CONST)
      {
